...
import { Course, getCourseBySlug, getTeacherById, getReviewsByCourse, courses } from '@/data/tuitionData';
...
export default function CourseDetailPage({ slug, onBack }: CourseDetailPageProps) {
  const { t } = useLanguage();
  ...
  const { totalLessons, freeLessons } = useMemo(() => ({
    totalLessons: course ? course.modules.reduce((acc, module) => acc + module.lessons.length, 0) : 0,
    freeLessons: course ? course.modules.reduce((acc, module) =>
      acc + module.lessons.filter(lesson => lesson.isFree).length, 0
    ) : 0
  }), [course]);
      ...
      <div className="container" style={{ maxWidth: '1400px', margin: '0 auto', padding: '2rem 1rem' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ cursor: 'pointer', color: 'var(--accent)' }}>Home</span>
            <span>›</span>
            <span
              onClick={onBack}
              style={{ cursor: onBack ? 'pointer' : 'default', color: onBack ? 'var(--accent)' : 'var(--muted)' }}
            >
              Tuition
            </span>
            <span>›</span>
            <span style={{ cursor: 'pointer', color: 'var(--accent)' }}>{course.category}</span>
            <span>›</span>
          ...
          <div>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.1rem' }}>
                  {[...Array(5)].map((_, i) => (
                    <span key={i} style={{ color: i < Math.floor(course.rating) ? '#f59e0b' : '#d1d5db' }}>
                      ⭐
                    </span>
                  ))}
                </div>
              ...
              <div style={{
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '1.1rem', fontWeight: 600, marginBottom: '0.25rem' }}>
                    {teacher.name}
                    {teacher.verified && <span style={{ marginLeft: '0.5rem', color: 'var(--accent)' }}>✓</span>}
                  </div>
                ...
                <div style={{
                  ...
                  <div>👥 {teacher.totalStudents.toLocaleString()} students</div>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ fontSize: '0.8rem', color: 'var(--muted)' }}>Total Duration</div>
              </div>
              ...
              <div style={{
                ...
                <div style={{ fontSize: '0.8rem', color: 'var(--muted)' }}>Lessons</div>
              </div>
              ...
              <div style={{
                ...
                <div style={{ fontSize: '0.8rem', color: 'var(--muted)' }}>Weeks</div>
              </div>
            ...
            <div style={{
              ...
              <button
                ...
                style={{
                  width: '100%',
                  padding: '1rem',
                  background: isEnrolled ? 'var(--muted)' : 'linear-gradient(135deg, var(--accent) 0%, #764ba2 100%)',
                  color: '#fff',
                  border: 'none',
                  borderRadius: '12px',
                  fontSize: '1.1rem',
                  ...
                  boxShadow: isEnrolled ? 'none' : '0 4px 15px rgba(102, 126, 234, 0.4)'
                }}
                onMouseEnter={(e) => {
                  if (!isEnrolled) e.currentTarget.style.transform = 'translateY(-2px)';
                }}
                onMouseLeave={(e) => {
                  if (!isEnrolled) e.currentTarget.style.transform = 'translateY(0)';
                }}
              >
              ...
              <div style={{
                ...
                <div style={{ fontSize: '0.95rem', fontWeight: 700, marginBottom: '1rem' }}>
                  This course includes:
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', fontSize: '0.9rem' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <span>📝</span>
                    <span>{totalLessons} lessons & {course.modules.length} modules</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <span>📱</span>
              ...
              <div>
                <div style={{
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '0.95rem', color: 'var(--muted)' }}>
                      {course.modules.length} modules • {totalLessons} lessons • {course.totalHours}h total
                      {freeLessons > 0 && (
                        <span style={{ marginLeft: '1rem', color: 'var(--accent)', fontWeight: 600 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  {course.modules.map((module, moduleIndex) => {
                    ...
                    const moduleDuration = module.lessons.reduce((acc, lesson) => acc + lesson.duration, 0);
                        ...
                        >
                          <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: '0.9rem', color: 'var(--muted)' }}>
                              {module.lessons.length} lessons • {Math.floor(moduleDuration / 60)}h {moduleDuration % 60}m
                            ...
                            {module.lessons.map((lesson, lessonIndex) => (
                              <div
                                ...
                                style={{
                                  ...
                                  justifyContent: 'space-between',
                                  alignItems: 'center',
                                  cursor: lesson.isFree || isEnrolled ? 'pointer' : 'default',
                                  opacity: lesson.isFree || isEnrolled ? 1 : 0.7,
                                  transition: 'background 0.2s ease'
                                }}
                                onMouseEnter={(e) => {
              ...
              <div>
                <div style={{
                  ...
                  <div style={{ display: 'flex', gap: '2rem', marginBottom: '2rem' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <h3 style={{ fontSize: '1.8rem', fontWeight: 700, marginBottom: '0.5rem' }}>
                        {teacher.name}
                        {teacher.verified && (
                          <span style={{ marginLeft: '0.75rem', color: 'var(--accent)', fontSize: '1.5rem' }}>
                            ✓
                          </span>
                        )}
                      </h3>
                      ...
                      <div style={{
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '0.85rem', color: 'var(--muted)' }}>Instructor Rating</div>
                        </div>
                        <div>
                          <div style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--accent)' }}>
                            {(teacher.totalStudents / 1000).toFixed(1)}K
                          </div>
                          <div style={{ fontSize: '0.85rem', color: 'var(--muted)' }}>Students</div>
                        </div>
                        <div>
                          <div style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--accent)' }}>
                            {teacher.totalCourses}
                          </div>
                          <div style={{ fontSize: '0.85rem', color: 'var(--muted)' }}>Courses</div>
                        </div>
                        <div>
                          <div style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--accent)' }}>
                            {teacher.yearsExperience}y
                          </div>
                          <div style={{ fontSize: '0.85rem', color: 'var(--muted)' }}>Experience</div>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ marginBottom: '2rem' }}>
                    <h4 style={{ fontSize: '1.2rem', fontWeight: 700, marginBottom: '1rem' }}>About</h4>
                    <p style={{ lineHeight: '1.8', color: 'var(--muted)' }}>{teacher.bio}</p>
                  </div>

                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
                    <div>
                      <h4 style={{ fontSize: '1.2rem', fontWeight: 700, marginBottom: '1rem' }}>Education</h4>
                    ...
                    <div>
                      <h4 style={{ fontSize: '1.2rem', fontWeight: 700, marginBottom: '1rem' }}>Specializations</h4>
                  ...
                  {teacher.certifications.length > 0 && (
                    <div style={{ marginTop: '2rem' }}>
                      <h4 style={{ fontSize: '1.2rem', fontWeight: 700, marginBottom: '1rem' }}>Certifications</h4>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                ...
                <div style={{
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: '3rem' }}>
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                        {[5, 4, 3, 2, 1].map(star => {
                          const percentage = Math.floor(Math.random() * 60 + (star >= 4 ? 30 : 10));
                          return (
                            <div key={star} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', fontSize: '0.9rem' }}>
                              ...
                              <div style={{ width: '40px', textAlign: 'right', color: 'var(--muted)' }}>
                                {percentage}%
                              </div>
                            </div>
                          );
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1rem' }}>
                        {[
                          ...
                          <div key={i} style={{
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                              <span style={{ fontSize: '1.5rem', fontWeight: 700 }}>{category.score}</span>
                              <div style={{ display: 'flex', gap: '0.1rem' }}>
                                {[...Array(5)].map((_, j) => (
                                  <span key={j} style={{
                                    fontSize: '0.8rem',
                                    color: j < Math.floor(category.score) ? '#f59e0b' : '#d1d5db'
                                  }}>
                                    ⭐
                                  </span>
                                ))}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                  {reviews.slice(0, 5).map((review) => (
                    ...
                    >
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.85rem', color: 'var(--muted)' }}>
                            <div style={{ display: 'flex', gap: '0.1rem' }}>
                              {[...Array(5)].map((_, i) => (
                                <span key={i} style={{
                                  fontSize: '0.9rem',
                                  color: i < Math.floor(review.rating) ? '#f59e0b' : '#d1d5db'
                                }}>
                                  ⭐
                                </span>
                              ))}
                      ...
                      <p style={{ lineHeight: '1.7', color: 'var(--fg)', marginBottom: '1rem' }}>
                        {review.comment}
                      </p>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    {[
                      ...
                      },
                      {
                        ...
                        answer: 'You have lifetime access to this course. Once enrolled, you can learn at your own pace with no time limits.'
                      },
                      {
                      ...
                      },
                      {
                        ...
                        answer: 'Yes, upon successful completion of all modules and assessments, you will receive a verified certificate that you can share on LinkedIn and your resume.'
                      },
                      {
                        question: 'What if I\'m not satisfied with the course?',
                        answer: 'We offer a 30-day money-back guarantee. If you\'re not satisfied with the course for any reason, you can request a full refund within 30 days of enrollment.'
                      },
                      {
                        question: 'Can I download the course videos?',
                        answer: 'Yes, all video lessons can be downloaded for offline viewing through our mobile app. You can learn anytime, anywhere without an internet connection.'
                      }
                    ].map((faq, i) => (
                      <div key={i} style={{
                        padding: '1.5rem',
