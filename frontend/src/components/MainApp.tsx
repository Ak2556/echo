...
const SocialMediaHub = dynamic(() => import('./SocialMediaHub'), {
  loading: () => <div className="flex items-center justify-center h-96"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500"></div></div>
});
...
function MainAppContent({ onOpenMiniApp }: MainAppProps) {
  ...
  const renderContent = () => {
    ...
    const fallback = currentRoute === 'feed' ? <FeedSkeleton count={2} /> : <PageSkeleton />;
  ...
  const enhancedHeader = isMobile || isTablet ? (
    <header
      style={{
        position: 'sticky',
        top: 0,
    ...
    <div className="p-4">
      ...
      <nav className="space-y-2">
        {[
          ...
          <button
            key={item.route}
            onClick={() => window.dispatchEvent(new CustomEvent('navigate', { detail: item.route }))}
  ...
  return (
    ...
    >
      <div className={currentRoute === 'feed' || currentRoute === 'messages' ? '' : 'p-6'}>
        {renderContent()}
      </div>
