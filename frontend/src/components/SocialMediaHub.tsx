...
type PostType = 'text' | 'image' | 'video' | 'story' | 'reel' | 'thread' | 'poll' | 'live';
type MessageType = 'text' | 'image' | 'video' | 'voice' | 'sticker' | 'gif' | 'location' | 'file';
...
export default function SocialMediaHub() {
  ...
  // State management
  const [activeTab, setActiveTab] = useState<'feed' | 'stories' | 'messages' | 'explore' | 'profile'>('feed');
  ...
  const initializeSampleData = () => {
    ...
    const users: User[] = [
      ...
      },
      {
        ...
        avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Kavya&backgroundColor=f0e6ff',
        verified: true,
        followers: 45000,
        following: 2100,
        isOnline: true,
    ...
    const samplePosts: Post[] = [
      {
        id: '1',
        author: users[0],
        content: 'Just finished this amazing digital artwork! What do you think? 🎨✨ #DigitalArt #CreativeProcess',
        type: 'image',
        media: {
          type: 'image',
          urls: ['https://images.unsplash.com/photo-1541961017774-22349e4a1262?w=800&h=600&fit=crop']
        },
      ...
      },
      {
        id: '2',
        author: users[1],
        content: 'Building something amazing with React and TypeScript! The developer experience is incredible 🚀',
        type: 'text',
      ...
      },
      {
        id: '3',
        author: users[2],
        content: 'Made this incredible South Indian feast today! Recipe thread below 👇🍛',
        type: 'video',
        media: {
          type: 'video',
          urls: ['https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4'],
          thumbnail: 'https://images.unsplash.com/photo-1565299624946-b28f40a0ca4b?w=800&h=600&fit=crop',
          duration: '2:34'
        },
    ...
    const sampleStories: Story[] = [
      ...
      },
      {
        ...
        media: {
          ...
          thumbnail: 'https://images.unsplash.com/photo-1517077304055-6e89abbf09b0?w=400&h=700&fit=crop'
        },
  ...
  const handleSendMessage = useCallback(() => {
    ...
    const message: Message = {
      ...
      sender: {
        ...
        username: 'you',
        displayName: 'You',
        avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=You&backgroundColor=a7e6ff',
        verified: false,
        followers: 0,
        following: 0,
        isOnline: true
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'stories':
        return <StoriesView stories={stories} selectedStory={selectedStory} setSelectedStory={setSelectedStory} />;
      case 'messages':
        return (
          <MessagesView 
            chats={chats}
  ...
  return (
    <div className="social-media-hub" style={{
      minHeight: '100vh',
...
// Feed View Component
const FeedView = ({ posts, setPosts }: { posts: Post[]; setPosts: React.Dispatch<React.SetStateAction<Post[]>> }) => {
  ...
  const [mediaFiles, setMediaFiles] = useState<{ type: 'image' | 'video' | 'audio'; url: string; file: File }[]>([]);
                        ...
                        <div key={idx} style={{ position: 'relative', borderRadius: '8px', overflow: 'hidden' }}>
                          {media.type === 'image' && (
                            <img
                              ...
                              style={{
                                width: '100%',
                                height: mediaFiles.length === 1 ? '300px' : '150px',
                                objectFit: 'cover'
                              }}
                            />
                          )}
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          borderRadius: '8px',
                          border: 'none',
                          background: (newPost.trim() || mediaFiles.length > 0) ? 'var(--accent)' : 'var(--bg-tertiary)',
                          color: (newPost.trim() || mediaFiles.length > 0) ? 'white' : 'var(--fg-muted)',
                          cursor: (newPost.trim() || mediaFiles.length > 0) ? 'pointer' : 'not-allowed',
                          fontWeight: 600
                        }}
                      >
                        Post
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  ...
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      ...
                      {post.author.verified && <span style={{ color: 'var(--accent)' }}>✓</span>}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.9rem', color: 'var(--fg-secondary)' }}>
                ...
                {post.poll && (
                  <div style={{
                    ...
                    <p style={{
                      ...
                      {post.poll.totalVotes.toLocaleString()} votes • Ends {new Date(post.poll.expiresAt).toLocaleDateString()}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', gap: '1rem' }}>
                  ...
                  <button
                    ...
                    borderRadius: '8px',
                    border: 'none',
                    background: commentingPostId === post.id ? 'rgba(59, 130, 246, 0.1)' : 'transparent',
                    color: commentingPostId === post.id ? '#3b82f6' : 'var(--fg-secondary)',
                    cursor: 'pointer',
                    fontSize: '0.9rem',
                    fontWeight: 500
                  }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button
                    ...
                    style={{
                      ...
                      border: 'none',
                      background: 'transparent',
                      color: post.isBookmarked ? 'var(--accent)' : 'var(--fg-secondary)',
                      cursor: 'pointer',
                      fontSize: '1.1rem'
                    }}
                  >
                      ...
                      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem', marginTop: '0.5rem' }}>
                        ...
                        <button
                          ...
                          style={{
                            ...
                            color: commentText.trim() ? 'white' : 'var(--fg-muted)',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {[
                ...
                },
                {
                  ...
                  avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Sneha&backgroundColor=ffdfba',
                  followers: '1.8K'
                }
              ].map((user, idx) => {
...
const StoriesView = memo(({ stories, selectedStory, setSelectedStory }: {
  stories: Story[];
        ...
        {stories.map(story => (
          <motion.div
            ...
            style={{
              ...
              overflow: 'hidden',
              cursor: 'pointer',
              border: story.isViewed ? '2px solid var(--border-primary)' : '2px solid var(--accent)'
            }}
          >
            <Image
          ...
          <div style={{
            ...
            {messages.map(message => (
              <motion.div
                ...
                style={{
                  display: 'flex',
                  justifyContent: message.sender.id === 'current_user' ? 'flex-end' : 'flex-start'
                }}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              >
                <div style={{
                  ...
                  padding: '0.75rem 1rem',
                  borderRadius: '16px',
                  background: message.sender.id === 'current_user' 
                    ? 'var(--accent)' 
                    : 'var(--bg-tertiary)',
                  color: message.sender.id === 'current_user' 
                    ? 'white' 
                    : 'var(--fg-primary)'
                }}>
                  {message.media && (
            ...
            <div style={{
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: '8px',
                  border: 'none',
                  background: newMessage.trim() ? 'var(--accent)' : 'var(--bg-secondary)',
            ...
            {[
              {
                ...
                followers: '125K',
                avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=TechGuru&backgroundColor=a7e6ff'
              },
              {
                name: 'Food Explorer',
                username: 'foodie_explorer',
                followers: '89K',
                avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=FoodExplorer&backgroundColor=ffdfba'
              }
            ].map((creator, idx) => (
              <div key={idx} style={{
                display: 'flex',
...
const ProfileView = memo(() => {
  ...
  const highlights = [
    { id: '1', name: 'Travel', cover: 'https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=100&h=100&fit=crop' },
    { id: '2', name: 'Food', cover: 'https://images.unsplash.com/photo-1565299624946-b28f40a0ca4b?w=100&h=100&fit=crop' },
    { id: '3', name: 'Tech', cover: 'https://images.unsplash.com/photo-1518770660439-4636190af475?w=100&h=100&fit=crop' },
    { id: '4', name: 'Art', cover: 'https://images.unsplash.com/photo-1541961017774-22349e4a1262?w=100&h=100&fit=crop' },
    { id: '5', name: 'Music', cover: 'https://images.unsplash.com/photo-1511379938547-c1f69419868d?w=100&h=100&fit=crop' }
  ];
  ...
  const userPosts = [
    {
      id: '1',
      type: 'image',
      media: 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=400&h=400&fit=crop',
      likes: 234,
      comments: 18
    },
    {
      id: '2',
      type: 'image',
      media: 'https://images.unsplash.com/photo-1541961017774-22349e4a1262?w=400&h=400&fit=crop',
      likes: 567,
      comments: 42
    },
    {
      id: '3',
      type: 'video',
      media: 'https://images.unsplash.com/photo-1517077304055-6e89abbf09b0?w=400&h=400&fit=crop',
      likes: 891,
      comments: 67
    },
    {
        ...
        <div style={{ flex: 1, textAlign: isMobile ? 'center' : 'left' }}>
          ...
          <div style={{
            ...
            <div>
              ...
              <span style={{ color: 'var(--fg-secondary)', marginLeft: '0.25rem' }}>Posts</span>
            </div>
            <div style={{ cursor: 'pointer' }}>
              <span style={{ fontWeight: 600 }}>{stats.followers.toLocaleString()}</span>
              <span style={{ color: 'var(--fg-secondary)', marginLeft: '0.25rem' }}>Followers</span>
            </div>
            <div style={{ cursor: 'pointer' }}>
              <span style={{ fontWeight: 600 }}>{stats.following}</span>
              <span style={{ color: 'var(--fg-secondary)', marginLeft: '0.25rem' }}>Following</span>
            </div>
          </div>
        ...
        <div style={{
          ...
          {highlights.map((highlight) => (
            ...
            >
              ...
              <span style={{ fontSize: '0.75rem', fontWeight: 500 }}>{highlight.name}</span>
            </div>
          ))}
          <div
            style={{
          ...
          >
            ...
            <span style={{ fontSize: '0.75rem', fontWeight: 500, color: 'var(--fg-secondary)' }}>New</span>
          </div>
        </div>
      </div>
        ...
        {[
          ...
          <button
            ...
            style={{
              padding: '1rem 0',
              border: 'none',
              borderTop: activeTab === tab.id ? '1px solid var(--fg-primary)' : '1px solid transparent',
              marginTop: '-1px',
              background: 'transparent',
