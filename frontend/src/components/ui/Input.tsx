...
const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ className, label, error, success, helperText, variant = 'modern', icon, iconPosition = 'left', showPasswordToggle, id, type, ...props }, ref) => {
      ...
      <div className="space-y-2 w-full">
        ...
        <div ref={inputWrapperRef} className="relative w-full">
          {icon && iconPosition === 'left' && (
            <div ref={iconRef} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500 pointer-events-none">
              {icon}
            </div>
          )}
          <input
            ...
            className={cn(
              ...
              // Success state
              success && '!border-green-500 dark:!border-green-400 focus:!border-green-500 focus:!shadow-[0_0_0_3px_rgba(34,197,94,0.1)]',

              // Icon padding
              icon && iconPosition === 'left' ? 'input-with-icon-left' : '',
              (icon && iconPosition === 'right' || (isPasswordType && showPasswordToggle)) ? 'input-with-icon-right' : '',
        ...
        {error && (
          <p ref={errorRef} id={`${inputId}-error`} className="text-sm font-medium text-red-600 dark:text-red-400 flex items-center gap-1">
        ...
        {success && !error && (
          <p ref={successRef} id={`${inputId}-success`} className="text-sm font-medium text-green-600 dark:text-green-400 flex items-center gap-1">
        ...
        {helperText && !error && !success && (
          <p id={`${inputId}-helper`} className="text-sm text-gray-500 dark:text-gray-400">{helperText}</p>
        )}
      </div>
    );
  }
