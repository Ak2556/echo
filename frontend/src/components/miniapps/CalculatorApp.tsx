...
const generateCalculationInsights = (
  history: HistoryEntry[]
): { text: string; tips: string[] } => {
  if (history.length === 0) {
    return {
      text: 'Start calculating to get insights!',
      tips: [
        'Try using scientific functions',
  ...
  const hasScientific = recentOps.some((h) =>
    /sin|cos|tan|log|sqrt/i.test(h.expression)
  );
...
const CalculatorApp = memo(({ isVisible, onClose }: CalculatorAppProps) => {
  ...
  const calculate = useCallback(
    (firstOperand: number, secondOperand: number, op: string) => {
      switch (op) {
        case '+':
    ...
    (exprRaw: string): number => {
        ...
        if (t.type === 'num' || t.type === 'var') {
          ...
          while (ops.length) {
            ...
            if (
              o2.type === 'func' ||
              (o2.type === 'op' &&
                ((prec[o2.value] || 0) > (prec[o1.value] || 0) ||
                  ((prec[o2.value] || 0) === (prec[o1.value] || 0) &&
                    !rightAssoc.has(o1.value))))
            ) {
      ...
      for (const tok of output) {
        if (tok.type === 'num') {
          ...
          switch (fn) {
            case 'sin':
              res = isDegreeMode ? Math.sin((a * Math.PI) / 180) : Math.sin(a);
              break;
            case 'cos':
              res = isDegreeMode ? Math.cos((a * Math.PI) / 180) : Math.cos(a);
              break;
            case 'tan':
              res = isDegreeMode ? Math.tan((a * Math.PI) / 180) : Math.tan(a);
              break;
            case 'asin':
              res = isDegreeMode
                ? (Math.asin(a) * 180) / Math.PI
                : Math.asin(a);
              break;
            case 'acos':
              res = isDegreeMode
            ...
            case 'atan':
              ...
              break;
            case 'ln':
              res = a <= 0 ? Infinity : Math.log(a);
              break;
            case 'log':
            case 'log10':
              res = a <= 0 ? Infinity : Math.log10(a);
              break;
            case 'log2':
              res = a <= 0 ? Infinity : Math.log2(a);
              break;
            case 'sqrt':
              res = a < 0 ? Infinity : Math.sqrt(a);
              break;
            case 'abs':
              res = Math.abs(a);
              break;
    ...
    try {
      ...
      if (previousValue !== null && operation) {
        ...
        switch (operation) {
          ...
          case '/':
            result =
              secondOperand === 0 ? Infinity : firstOperand / secondOperand;
            break;
          case '^':
  ...
  const backspace = useCallback(() => {
    if (hasError) return;
    setDisplay((d) =>
      d.length <= 1 || (d.length === 2 && d.startsWith('-'))
        ? '0'
        : d.slice(0, -1)
    );
  }, [hasError]);
    ...
    (fn: string) => {
      ...
      switch (fn) {
        ...
        case '1/x':
          ...
          break;
        case 'sin':
          v = isDegreeMode ? Math.sin((x * Math.PI) / 180) : Math.sin(x);
          break;
        case 'cos':
          v = isDegreeMode ? Math.cos((x * Math.PI) / 180) : Math.cos(x);
          break;
        case 'tan':
          v = isDegreeMode ? Math.tan((x * Math.PI) / 180) : Math.tan(x);
          break;
        case 'asin':
          v = isDegreeMode ? (Math.asin(x) * 180) / Math.PI : Math.asin(x);
          break;
        case 'acos':
          v = isDegreeMode ? (Math.acos(x) * 180) / Math.PI : Math.acos(x);
          break;
        case 'atan':
          v = isDegreeMode ? (Math.atan(x) * 180) / Math.PI : Math.atan(x);
          break;
        case 'ln':
          v = x <= 0 ? Infinity : Math.log(x);
          break;
        case 'log10':
          v = x <= 0 ? Infinity : Math.log10(x);
          break;
        case 'log2':
          v = x <= 0 ? Infinity : Math.log2(x);
          break;
        case 'exp':
          v = Math.exp(x);
          break;
  ...
  const convertUnits = useMemo(() => {
    ...
    if (converterType === 'temperature') {
      ...
      const result =
        tempConversions[toUnit as keyof typeof tempConversions](celsius);
      return formatDisplay(result);
    } else if (converterType === 'weight') {
      const kgValue =
        value /
        UNIT_CONVERSIONS.weight[
          fromUnit as keyof typeof UNIT_CONVERSIONS.weight
        ];
      const result =
        kgValue *
        UNIT_CONVERSIONS.weight[toUnit as keyof typeof UNIT_CONVERSIONS.weight];
      return formatDisplay(result);
    } else {
      const meterValue =
        value /
        UNIT_CONVERSIONS.length[
          fromUnit as keyof typeof UNIT_CONVERSIONS.length
        ];
      const result =
        meterValue *
        UNIT_CONVERSIONS.length[toUnit as keyof typeof UNIT_CONVERSIONS.length];
      return formatDisplay(result);
    }
  ...
  const aiInsights = useMemo(
    () => generateCalculationInsights(history),
    [history]
  );
  ...
  const calcStats = useMemo(() => {
    ...
    const avgResultLength =
      history.length > 0
        ? (
            history.reduce((sum, h) => sum + h.result.length, 0) /
            history.length
          ).toFixed(1)
        : '0';
      ...
      >
        ...
        {mode === 'converter' && (
          <div>
            ...
            >
              ...
              >
                {(['length', 'weight', 'temperature'] as UnitType[]).map(
                  (type) => (
                    <button
                      key={type}
                      onClick={() => {
        ...
        {(mode === 'basic' || mode === 'scientific') && (
          <div>
                ...
                {[
                  {
                    ...
                    action: () => setIsDegreeMode(!isDegreeMode),
                  },
            ...
            >
              {/* Row 1 */}
              <button onClick={inputMemoryClear} style={memBtnStyle}>
                MC
              </button>
              <button onClick={inputMemoryRecall} style={memBtnStyle}>
                MR
              </button>
              <button onClick={inputMemoryAdd} style={memBtnStyle}>
                M+
              </button>
              <button onClick={() => inputOperation('÷')} style={opBtnStyle}>
                ÷
              </button>

              {/* Row 2 */}
              <button onClick={clearAll} style={funcBtnStyle}>
                C
              </button>
              <button onClick={clearEntry} style={funcBtnStyle}>
                CE
              </button>
              <button onClick={toggleSign} style={funcBtnStyle}>
                ±
              </button>
              <button onClick={() => inputOperation('×')} style={opBtnStyle}>
                ×
              ...
              {/* Row 3 */}
              <button onClick={() => inputNumber('7')} style={numBtnStyle}>
                7
              </button>
              <button onClick={() => inputNumber('8')} style={numBtnStyle}>
                8
              </button>
              <button onClick={() => inputNumber('9')} style={numBtnStyle}>
                9
              </button>
              <button onClick={() => inputOperation('-')} style={opBtnStyle}>
                −
              ...
              {/* Row 4 */}
              <button onClick={() => inputNumber('4')} style={numBtnStyle}>
                4
              </button>
              <button onClick={() => inputNumber('5')} style={numBtnStyle}>
                5
              </button>
              <button onClick={() => inputNumber('6')} style={numBtnStyle}>
                6
              </button>
              <button onClick={() => inputOperation('+')} style={opBtnStyle}>
                +
              ...
              {/* Row 5 */}
              <button onClick={() => inputNumber('1')} style={numBtnStyle}>
                1
              </button>
              <button onClick={() => inputNumber('2')} style={numBtnStyle}>
                2
              </button>
              <button onClick={() => inputNumber('3')} style={numBtnStyle}>
                3
              </button>
              <button
                onClick={evaluateEquals}
                style={{ ...equalsBtnStyle, gridRow: 'span 2' }}
              >
                =
              </button>
              ...
              <button
                ...
                style={{ ...numBtnStyle, gridColumn: 'span 2' }}
              >
                0
              </button>
              <button onClick={() => inputNumber('.')} style={numBtnStyle}>
                .
              </button>
            </div>
          </div>
...
const equalsBtnStyle: React.CSSProperties = {
  ...baseBtnStyle,
  background:
    'linear-gradient(135deg, colors.status.success 0%, colors.chart[1] 100%)',
  color: 'white',
