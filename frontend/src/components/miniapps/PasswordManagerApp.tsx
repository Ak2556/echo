...
const COMMON_PASSWORDS = ['password', '123456', 'qwerty', 'admin', 'letmein', 'welcome'];

// Categories
const CATEGORIES = [
  { id: 'all', name: 'All Items', icon: 'folder', color: 'colors.brand.primary' },
  { id: 'logins', name: 'Logins', icon: 'key', color: 'colors.status.info' },
  { id: 'cards', name: 'Credit Cards', icon: 'card', color: 'colors.status.warning' },
  { id: 'identity', name: 'Identity', icon: 'id', color: 'colors.status.success' },
  { id: 'notes', name: 'Secure Notes', icon: 'note', color: 'colors.brand.primary' },
];

export default function PasswordManagerApp({ isVisible, onClose }: PasswordManagerAppProps) {
  const colors = useThemeColors();
  // State
  const [activeTab, setActiveTab] = useState<'vault' | 'generator' | 'analyzer'>('vault');
  ...
  const analyzeStrength = useCallback((password: string): { score: number; label: string; color: string; crackTime: string } => {
    if (!password) {
      return { score: 0, label: 'None', color: '#6b7280', crackTime: 'Instant' };
    }
    ...
    const noCommon = !COMMON_PASSWORDS.some(p => password.toLowerCase().includes(p));
    ...
    let label: string;
    let color: string;
    if (score >= 8) { label = 'Excellent'; color = '#059669'; }
    else if (score >= 6) { label = 'Strong'; color = 'colors.status.success'; }
    else if (score >= 4) { label = 'Good'; color = 'colors.status.warning'; }
    else if (score >= 2) { label = 'Fair'; color = '#f97316'; }
    else { label = 'Weak'; color = 'colors.status.error'; }
  ...
  const securityScore = useMemo((): SecurityScore => {
    if (entries.length === 0) {
      return { overall: 100, weakPasswords: 0, reusedPasswords: 0, oldPasswords: 0, totalEntries: 0 };
    }

    const weakPasswords = entries.filter(e => e.strength < 5).length;
    const reusedPasswords = entries.length - new Set(entries.map(e => e.password)).size;
  ...
  const generateSecurityInsight = useCallback((): string => {
    const { overall, weakPasswords, reusedPasswords, oldPasswords, totalEntries } = securityScore;
    ...
    if (overall >= 90) {
      insights.push("Excellent security! Your password vault is well-protected.");
    } else if (overall >= 70) {
      insights.push("Good security posture. A few improvements would strengthen your vault.");
    } else if (overall >= 50) {
      insights.push("Moderate security. Consider updating weak passwords soon.");
    } else {
      insights.push("Security needs attention! Multiple passwords require updates.");
    }

    // Specific insights
    if (weakPasswords > 0) {
      insights.push(`${weakPasswords} password${weakPasswords > 1 ? 's are' : ' is'} weak. Use the generator for stronger ones.`);
    }

    if (reusedPasswords > 0) {
      insights.push(`${reusedPasswords} password${reusedPasswords > 1 ? 's are' : ' is'} reused. Each account needs a unique password.`);
    }

    if (oldPasswords > 0) {
      insights.push(`${oldPasswords} password${oldPasswords > 1 ? 's haven\'t' : ' hasn\'t'} been updated in 90+ days.`);
    }

    if (totalEntries >= 10 && weakPasswords === 0 && reusedPasswords === 0) {
      insights.push("Great job maintaining unique, strong passwords across all accounts!");
    }
  ...
  const addEntry = useCallback(() => {
    ...
    setNewEntry({ title: '', username: '', password: '', url: '', notes: '', category: 'logins' });
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <span style={{ fontSize: '1.25rem', color: 'white' }}>Password Manager</span>
          <span style={{
            fontSize: '0.7rem',
          ...
          <div style={{
            ...
            <span style={{ fontSize: '0.7rem', color: 'rgba(255, 255, 255, 0.7)' }}>Security</span>
          </div>
        </div>
        ...
        {showAI && (
          <div style={{
            ...
            <div style={{
              ...
              {[
                { label: 'Total', value: securityScore.totalEntries, color: 'colors.brand.primary' },
                { label: 'Weak', value: securityScore.weakPasswords, color: 'colors.status.error' },
                { label: 'Reused', value: securityScore.reusedPasswords, color: 'colors.status.warning' },
                { label: 'Old', value: securityScore.oldPasswords, color: 'colors.brand.primary' },
              ].map(stat => (
          ...
          <div>
            ...
            <div style={{
              ...
              {CATEGORIES.map(cat => (
                ...
                >
                  {cat.name} ({cat.id === 'all' ? entries.length : entries.filter(e => e.category === cat.id).length})
                </button>
              ))}
            </div>
            ...
            {filteredEntries.length === 0 ? (
              <div style={{
                ...
                <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>Key icon</div>
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        padding: '0.75rem',
                        cursor: 'pointer',
                        borderColor: selectedEntry?.id === entry.id ? 'colors.brand.primary' : 'rgba(255, 255, 255, 0.1)'
                      }}
                    >
                      <div style={{
                        display: 'flex',
                        ...
                        <div style={{
                          ...
                          height: '36px',
                          borderRadius: '8px',
                          background: CATEGORIES.find(c => c.id === entry.category)?.color || 'colors.brand.primary',
                          display: 'flex',
                          alignItems: 'center',
                          justifyContent: 'center',
                          color: 'white',
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{
                            ...
                            {entry.favorite && <span style={{ fontSize: '0.7rem' }}>*</span>}
                            <span style={{
                              fontSize: '0.9rem',
                              fontWeight: '500',
                              color: 'white',
                        ...
                        <button
                          ...
                          style={{
                            ...
                            padding: '0.4rem 0.6rem',
                            borderRadius: '6px',
                            color: copiedField === entry.id ? 'colors.status.success' : 'rgba(255, 255, 255, 0.6)',
                            cursor: 'pointer',
                            fontSize: '0.75rem'
                          }}
                        >
                        ...
                        <button
                          ...
                          style={{
                            ...
                            padding: '0.25rem',
                            cursor: 'pointer',
                            color: entry.favorite ? 'colors.status.warning' : 'rgba(255, 255, 255, 0.4)',
                            fontSize: '0.85rem'
                          }}
                        >
                          Fav
                      ...
                      {selectedEntry?.id === entry.id && (
                        <div style={{
                          ...
                          <div style={{ marginBottom: '0.5rem' }}>
                            <label style={{ fontSize: '0.7rem', color: 'rgba(255, 255, 255, 0.5)' }}>Password</label>
                            <div style={{
                              display: 'flex',
                              alignItems: 'center',
                              gap: '0.5rem'
                          ...
                          {entry.url && (
                            <div style={{ marginBottom: '0.5rem' }}>
                              <label style={{ fontSize: '0.7rem', color: 'rgba(255, 255, 255, 0.5)' }}>URL</label>
                              <div style={{ fontSize: '0.85rem', color: 'colors.status.info' }}>{entry.url}</div>
                            </div>
                          )}
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center' }}>
                ...
                <button
                  ...
                  style={{
                    padding: '0.75rem 1rem',
                    background: generatedPassword ? 'colors.status.success' : 'rgba(255, 255, 255, 0.1)',
                    border: 'none',
                    borderRadius: '8px',
                    color: 'white',
                ...
                {[
                  ...
                  <label key={opt.key} style={{
                    ...
                    <input
                      ...
                      onChange={e => setGeneratorSettings(prev => ({ ...prev, [opt.key]: e.target.checked }))}
                    />
                    {opt.label}
                  </label>
                ))}
        ...
        {activeTab === 'analyzer' && (
          <div>
            <div style={{
              ...
              />
              <button
                onClick={() => setShowPassword(prev => ({ ...prev, analyzer: !prev.analyzer }))}
                style={{
                  marginLeft: '0.5rem',
                  padding: '0.75rem',
                  background: 'rgba(255, 255, 255, 0.1)',
                    ...
                    <>
                      ...
                      <div style={{
                        ...
                        <div style={{
                          ...
                          Time to crack: <strong style={{ color: 'white' }}>{result.crackTime}</strong>
                        </div>
                      </div>
                      ...
                      <div style={{
                        ...
                        {[
                          { check: analyzePassword.length >= 12, label: 'At least 12 characters' },
                          { check: /[A-Z]/.test(analyzePassword), label: 'Contains uppercase' },
                          { check: /[a-z]/.test(analyzePassword), label: 'Contains lowercase' },
                          { check: /[0-9]/.test(analyzePassword), label: 'Contains numbers' },
                          { check: /[^a-zA-Z0-9]/.test(analyzePassword), label: 'Contains symbols' },
                          { check: !COMMON_PASSWORDS.some(p => analyzePassword.toLowerCase().includes(p)), label: 'No common words' },
                        ].map((item, i) => (
                          <div key={i} style={{
                            display: 'flex',
                            alignItems: 'center',
                            ...
                            <span style={{
                              color: item.check ? 'colors.status.success' : 'colors.status.error',
                              fontSize: '0.85rem'
                            }}>
