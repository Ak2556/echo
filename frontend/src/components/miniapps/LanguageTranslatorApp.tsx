...
export default function LanguageTranslatorApp({
  isVisible,
  onClose,
}: LanguageTranslatorAppProps) {
  ...
  const [activeTab, setActiveTab] = useState<'translate' | 'history' | 'stats'>(
    'translate'
  );
  ...
  const aiTips = useMemo(() => {
    ...
    if (inputText.length > 0) {
      if (inputText.length > 500)
        tips.push(
          'Consider breaking long texts into smaller paragraphs for better accuracy.'
        );
      if (inputText.includes('...'))
        tips.push('Avoid using ellipses for more precise translations.');
      if (inputText === inputText.toUpperCase() && inputText.length > 10)
        tips.push('Using all caps may affect translation context.');
      if (/[!?]{2,}/.test(inputText))
        tips.push('Multiple punctuation marks may not translate well.');
    }
    if (tips.length === 0)
      tips.push('Type naturally for best translation results.');
    return tips;
  }, [inputText]);
  ...
  const stats = useMemo(() => {
    ...
    const wordCount = history.reduce(
      (acc, h) => acc + h.original.split(' ').length,
      0
    );
    ...
    const mostUsedPair = Object.entries(langPairs).sort(
      (a, b) => b[1] - a[1]
    )[0];
    ...
    return {
      ...
      avgLength:
        totalTranslations > 0 ? Math.round(wordCount / totalTranslations) : 0,
    };
  }, [history]);
  ...
  const handleTranslate = useCallback(async () => {
    ...
    const result =
      translations[sourceLang]?.[targetLang] ||
        ...
        {activeTab === 'stats' && (
          ...
          >
            ...
            >
              ...
              <div
                style={{ fontSize: '1.1rem', color: 'colors.brand.primary' }}
              >
                {stats.mostUsedPair !== 'N/A' ? (
                  <>
                    {getLangFlag(stats.mostUsedPair.split('-')[0])}{' '}
                    {getLangName(stats.mostUsedPair.split('-')[0])} →{' '}
                    {getLangFlag(stats.mostUsedPair.split('-')[1])}{' '}
                    {getLangName(stats.mostUsedPair.split('-')[1])}
                  </>
                ) : (
                  'No data yet'
                )}
