...
export default function TimerApp({ isVisible, onClose }: TimerAppProps) {
  ...
  const productivityStats = useMemo((): ProductivityStats => {
    ...
    const totalFocusTime = allFocusSessions.reduce(
  ...
  const generateProductivityInsight = useCallback((): string => {
    ...
    if (streakDays >= 7) {
      insights.push(
        `Amazing ${streakDays}-day streak! You're building strong focus habits.`
      );
    ...
    if (pomodorosCompleted >= 10) {
      insights.push(
        `You've completed ${pomodorosCompleted} Pomodoros! Impressive dedication.`
      );
    } else if (pomodorosCompleted > 0) {
      insights.push(
        `${pomodorosCompleted} Pomodoros done. Try to hit 4 per day for optimal flow.`
      );
    }
    ...
    if (hours >= 10) {
      insights.push(
        `${hours}+ hours of deep work logged. You're a focus champion!`
      );
    } else if (hours >= 1) {
      insights.push(
        `${hours} hour${hours > 1 ? 's' : ''} of focus time. Every session counts!`
      );
    }
    ...
    if (avgSessionLength >= 45) {
      insights.push(
        'Your average session is 45+ minutes - great for deep work!'
      );
    } else if (avgSessionLength >= 25) {
      insights.push(
        'Perfect Pomodoro-style sessions. Ideal for sustained focus.'
      );
    ...
    return (
      ...
      'Keep tracking your time to unlock productivity insights!'
    );
  ...
  const handleTimerComplete = useCallback(() => {
    ...
    if ('Notification' in window && Notification.permission === 'granted') {
      new Notification('Timer Complete!', {
        body:
          mode === 'pomodoro'
            ? `${pomodoroPhase} session finished`
      ...
      >
            ...
            {[
              ...
              >
                ...
                <input
                  ...
                  onChange={(e) =>
                    setting.setter(parseInt(e.target.value) * 60)
                  }
                  min="1"
                  max="60"
                  style={{
          ...
          >
            ...
            >
              ...
              <button
                ...
                style={{
                  background:
                    'linear-gradient(135deg, colors.status.info 0%, #2563eb 100%)',
                  border: 'none',
                  borderRadius: '6px',
          ...
          >
            ...
            <div
              style={{
                ...
                borderRadius: '50%',
                background:
                  'radial-gradient(circle at 30% 30%, rgba(102, 126, 234, 0.3) 0%, rgba(118, 75, 162, 0.15) 40%, transparent 70%)',
                filter: 'blur(30px)',
              }}
            />
        ...
        {mode === 'timer' && !isRunning && time === 0 && (
          ...
          >
            ...
            />
            <span
              style={{ fontSize: '1.5rem', color: 'rgba(255, 255, 255, 0.6)' }}
            >
              :
            </span>
            <input
        ...
        >
          {!isRunning ? (
            <button
              onClick={startTimer}
              disabled={
                mode === 'timer' && time === 0 && !inputMinutes && !inputSeconds
              }
              style={{
            ...
            <button
              ...
              style={{
                padding: '0.875rem 1.5rem',
                background:
                  'linear-gradient(135deg, colors.status.warning 0%, #d97706 100%)',
                border: 'none',
                borderRadius: '12px',
                color: 'white',
                fontWeight: '600',
            ...
            >
              {laps.map((lap, index) => (
                ...
                >
                  ...
                  <span
                    style={{ color: 'colors.status.info', fontWeight: '600' }}
                  >
