...
export function AccessibilitySettings({ onBack }: { onBack?: () => void }) {
  ...
  return (
    <SettingsSubPage title="Accessibility" description="Customize accessibility features and preferences" onBack={onBack}>
      <SettingsSection title="Visual Preferences" description="Adjust visual settings for better readability">
        <ToggleSwitch checked={reducedMotion} onChange={setReducedMotion} label="Reduce Motion" description="Minimize animations and transitions" />
        <ToggleSwitch checked={highContrast} onChange={setHighContrast} label="High Contrast Mode" description="Increase contrast for better visibility" />
        <div style={{ marginTop: 'var(--settings-space-6)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 'var(--settings-space-2)' }}>
            <span style={{ fontSize: 'var(--settings-text-base)', fontWeight: 'var(--settings-weight-medium)' }}>Font Scale</span>
            <span style={{ fontSize: 'var(--settings-text-sm)', color: 'var(--echo-text-secondary)' }}>{fontScale}%</span>
          </div>
          <input type="range" min="80" max="150" step="10" value={fontScale} onChange={(e) => setFontScale(parseInt(e.target.value))} style={{ width: '100%' }} />
        </div>
      </SettingsSection>

      <SettingsSection title="Navigation & Input" description="Control how you interact with the app">
        <ToggleSwitch checked={keyboardNav} onChange={setKeyboardNav} label="Keyboard Navigation" description="Enable full keyboard navigation support" />
        <ToggleSwitch checked={focusIndicators} onChange={setFocusIndicators} label="Focus Indicators" description="Show visible focus indicators for interactive elements" />
        <ToggleSwitch checked={screenReader} onChange={setScreenReader} label="Screen Reader Optimization" description="Optimize content for screen readers" />
      </SettingsSection>
