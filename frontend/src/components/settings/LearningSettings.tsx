...
export function LearningSettings({ onBack }: { onBack?: () => void }) {
  const [courseReminders, setCourseReminders] = useState(true);
  const [certificates, setCertificates] = useState(true);
  return (
    <SettingsSubPage title="Learning & Education" description="Manage tuition and educational content settings" onBack={onBack}>
      <SettingsSection title="Course Preferences"><ToggleSwitch checked={courseReminders} onChange={setCourseReminders} label="Course Reminders" description="Get reminded about upcoming lessons" /><ToggleSwitch checked={certificates} onChange={setCertificates} label="Certificate Notifications" description="Notify when certificates are available" /></SettingsSection>
