...
export function LiveSettings({ onBack }: { onBack?: () => void }) {
  const [liveNotifications, setLiveNotifications] = useState(true);
  const [chatEnabled, setChatEnabled] = useState(true);
  return (
    <SettingsSubPage title="Live Streams" description="Configure live streaming and video settings" onBack={onBack}>
      <SettingsSection title="Stream Preferences"><ToggleSwitch checked={liveNotifications} onChange={setLiveNotifications} label="Live Notifications" description="Alert when followed streamers go live" /><ToggleSwitch checked={chatEnabled} onChange={setChatEnabled} label="Chat Enabled" description="Show chat during live streams" /></SettingsSection>
