...
export function ShoppingSettings({ onBack }: { onBack?: () => void }) {
  const [savePayment, setSavePayment] = useState(true);
  const [priceAlerts, setPriceAlerts] = useState(true);
  return (
    <SettingsSubPage title="Shopping" description="Manage shopping preferences and payment methods" onBack={onBack}>
      <SettingsSection title="Payment & Checkout"><ToggleSwitch checked={savePayment} onChange={setSavePayment} label="Save Payment Methods" description="Securely store payment information" /><ToggleSwitch checked={priceAlerts} onChange={setPriceAlerts} label="Price Drop Alerts" description="Get notified when prices drop" /></SettingsSection>
