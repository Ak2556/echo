...
export function FeedSettings({ onBack }: { onBack?: () => void }) {
  ...
  const algorithmOptions = [
    { id: 'chronological', name: 'Chronological', icon: Clock, description: 'Latest posts first' },
    { id: 'recommended', name: 'Recommended', icon: Sparkles, description: 'Personalized for you' },
    { id: 'mixed', name: 'Mixed', icon: Shuffle, description: 'Blend of both' },
  ];

  return (
    <SettingsSubPage title="Feed Settings" description="Customize your feed and discovery preferences" onBack={onBack}>
      <SettingsSection title="Feed Algorithm" description="Choose how posts are sorted">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 'var(--settings-space-4)' }}>
          {algorithmOptions.map((option) => {
            const Icon = option.icon;
            const isActive = feedAlgorithm === option.id;
            return (
              <button key={option.id} onClick={() => setFeedAlgorithm(option.id)} style={{ padding: 'var(--settings-space-6)', borderRadius: 'var(--settings-radius-md)', border: isActive ? '2px solid var(--echo-primary)' : '1px solid var(--echo-border-light)', background: isActive ? 'rgba(0, 102, 255, 0.05)' : 'var(--echo-bg-primary)', cursor: 'pointer', transition: 'var(--settings-transition-normal)', textAlign: 'left' }}>
      ...
      <SettingsSection title="Content Preferences" description="Control what appears in your feed">
        <ToggleSwitch checked={suggestedPosts} onChange={setSuggestedPosts} label="Suggested Posts" description="Show posts you might like" />
        <ToggleSwitch checked={hideSeenPosts} onChange={setHideSeenPosts} label="Hide Seen Posts" description="Don't show posts you've already seen" />
        <ToggleSwitch checked={showReposts} onChange={setShowReposts} label="Show Reposts" description="Include reposts in your feed" />
      </SettingsSection>
