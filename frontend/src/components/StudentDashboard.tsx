...
export default function StudentDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    'all' | 'in-progress' | 'completed' | 'wishlist'
  >('all');
  ...
  const totalHoursLearned = enrolledCourses.reduce(
    (acc, c) => acc + c.hoursCompleted,
    0
  );
  const totalCoursesCompleted = enrolledCourses.filter(
    (c) => c.progress === 100
  ).length;
  const totalCertificates = enrolledCourses.filter(
    (c) => c.certificateEarned
  ).length;
                          ...
                          style={{
                            ...
                            background:
                              course.progress === 100
                                ...
                                : 'linear-gradient(135deg, var(--accent) 0%, #764ba2 100%)',
                            color:
                              course.progress === 100 ? 'var(--fg)' : '#fff',
                            border:
                              course.progress === 100
