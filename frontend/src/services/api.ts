...
class ApiService {
  ...
  async chat(
    ...
    try {
      ...
      if (
        ...
        return this.getContextualMockResponse(
          message.content,
          conversationHistory || [],
          settings
        );
  ...
  private getContextualMockResponse(
    userMessage: string,
    conversationHistory: ChatMessage[],
    settings?: AISettings
  ): Promise<{ response: string; image_url?: string; type?: string }> {
    ...
    const isOnFeed =
      currentPath.includes('feed') || window.location.hash.includes('feed');
    const isOnHome =
      currentPath === '/' ||
        ...
        setTimeout(
          () => {
            resolve({
              response:
                mockImageResponses[
                  Math.floor(Math.random() * mockImageResponses.length)
                ],
              type: 'text',
            });
          },
    ...
    // Contextual follow-ups based on conversation history
      ...
      hasDiscussedEcho &&
      (lowerMessage.includes('more') || lowerMessage.includes('tell me'))
    ) {
  ...
  private inferUserPreferences(
    ...
    const formalIndicators = userMessages.filter(
      (msg) =>
    ...
    const technicalTerms = userMessages.filter(
      (msg) =>
  ...
  async getAvailablePersonalities(): Promise<{
        ...
        personalities: [
          {
            id: 'helpful',
            name: 'Helpful',
            description: 'Supportive and informative',
          },
          {
            id: 'casual',
            name: 'Casual',
            description: 'Laid-back and friendly',
          },
          {
            id: 'professional',
            name: 'Professional',
          ...
          },
          {
            id: 'creative',
            name: 'Creative',
            description: 'Imaginative and enthusiastic',
          },
          {
            id: 'technical',
            name: 'Technical',
