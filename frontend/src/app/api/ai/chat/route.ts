...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      '🔑 API Key status:',
      apiKey
        ? `Present (${apiKey.substring(0, 10)}...)`
        : 'Missing - Running in DEMO mode'
    );
    ...
    if (isDemoMode) {
      ...
      if (lowerMessage.includes('hello') || lowerMessage.includes('hi')) {
        response =
          "👋 Hello! I'm Echo AI in demo mode. I can see you're greeting me! To enable intelligent conversations, please add your OPENROUTER_API_KEY to the environment configuration.";
      } else if (lowerMessage.includes('help')) {
        response =
          "I'm here to help! Currently running in demo mode. To enable full AI assistance with real-time intelligent responses, you'll need to configure an OPENROUTER_API_KEY in your .env file.";
      } else if (lowerMessage.includes('weather')) {
        response =
          "☀️ I'd love to help with weather information! In demo mode, I can't access real-time data. Configure your API key for live weather updates and more!";
      } else if (
