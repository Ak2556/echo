...
export function useAccessibilitySettings() {
  const { announce, updateSettings: updateContextSettings } =
  ...
  const saveSettings = useCallback(
    (newSettings: AccessibilitySettings) => {
      localStorage.setItem(
        'echo_accessibility_settings_v2',
  ...
  const playUISound = useCallback(
    (type: 'click' | 'hover' | 'error' | 'success' | 'notification') => {
      ...
      gainNode.gain.setValueAtTime(
        (settings.uiSoundsVolume / 100) * 0.1,
        audioContext.currentTime
      );
  ...
  const runAccessibilityTest = useCallback(
    async (
      testType: 'contrast' | 'keyboard' | 'screenreader' | 'focus' | 'full'
    ) => {
