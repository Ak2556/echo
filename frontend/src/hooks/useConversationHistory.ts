...
export function useConversationHistory() {
  ...
  const createNewConversation = useCallback((model: string, personality: string) => {
  ...
  const toggleBookmark = useCallback((id: string) => {
    updateConversation(id, { 
      isBookmarked: !history.conversations.find(c => c.id === id)?.isBookmarked 
    });
  }, [updateConversation, history.conversations]);

  const addTagToConversation = useCallback((id: string, tag: string) => {
  ...
  const removeTagFromConversation = useCallback((id: string, tag: string) => {
    const conversation = history.conversations.find(c => c.id === id);
    if (conversation) {
      updateConversation(id, {
  ...
  const getCurrentConversation = useCallback(() => {
    return history.conversations.find(c => c.id === history.currentConversationId) || null;
