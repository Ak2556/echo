...
from .models import User, Credential, RefreshToken, Session, VerificationCode, AuditLog
from .security import (
    hash_password, verify_password, generate_otp, hash_token,
    generate_totp_secret, get_totp_provisioning_uri, generate_qr_code,
    verify_totp, generate_backup_codes, verify_backup_code,
    check_password_breach, calculate_password_strength,
    generate_device_fingerprint, parse_user_agent
)
...
async def register(
    ...
    if redis and settings.environment != "test" and hasattr(redis, 'client') and redis.client:
        ...
        is_allowed, remaining = await redis.check_rate_limit(rate_key, max_requests=5, window_seconds=3600)
        if not is_allowed:
    ...
    if existing_user:
        await log_audit_event(db, None, data.email, "register_attempt", "failure", request, metadata={"reason": "email_exists"})
...
async def verify_email(
    ...
    if settings.debug or settings.environment == "development":
        if data.code in ["000000", "123456"]:
            ...
            if redis and hasattr(redis, 'client') and redis.client:
                is_valid, attempts_remaining = await redis.verify_code(data.email, data.code, "email_verify", max_attempts=3)
            else:
    ...
    if not is_valid:
        await log_audit_event(db, None, data.email, "email_verify", "failure", request, metadata={"attempts_remaining": attempts_remaining})
        raise HTTPException(
            status_code=400,
...
async def login(
    ...
    if redis and settings.environment != "test" and hasattr(redis, 'client') and redis.client:
        ...
        is_allowed, remaining = await redis.check_rate_limit(rate_key, max_requests=10, window_seconds=300)
        if not is_allowed:
    ...
    if not user:
        ...
        await log_audit_event(db, None, data.email, "login", "failure", request, "password", {"reason": "user_not_found"})
        raise HTTPException(status_code=401, detail="Invalid credentials")

    if user.is_locked:
        if user.locked_until and user.locked_until > datetime.now(timezone.utc):
            await log_audit_event(db, user.id, user.email, "login", "failure", request, "password", {"reason": "account_locked"})
    ...
    if not credential or not verify_password(credential.password_hash, data.password):
        ...
        await db.commit()
        await log_audit_event(db, user.id, user.email, "login", "failure", request, "password", {"reason": "invalid_password"})
        raise HTTPException(status_code=401, detail="Invalid credentials")
    ...
    if user.totp_enabled:
        ...
        if redis and hasattr(redis, 'client') and redis.client:
            await redis.cache_set(f"2fa_pending:{temp_token}", {"user_id": user.id}, expires_in_seconds=300)
    ...
    device_fp = generate_device_fingerprint(request.headers.get("user-agent", ""), request.client.host)
...
async def request_password_reset(
    ...
    if redis and settings.environment != "test" and hasattr(redis, 'client') and redis.client:
        ...
        is_allowed, remaining = await redis.check_rate_limit(rate_key, max_requests=3, window_seconds=3600)
        if not is_allowed:
    ...
    if user:
        ...
        if redis and hasattr(redis, 'client') and redis.client:
            await redis.store_verification_code(user.email, reset_token, "password_reset", expires_in_seconds=3600)

        logger.info("Password reset requested", user_id=user.id, email=user.email, token=reset_token)
        
        await log_audit_event(db, user.id, user.email, "password_reset_request", "success", request)
    else:
        await log_audit_event(db, None, data.email, "password_reset_request", "failure", request, metadata={"reason": "user_not_found"})
