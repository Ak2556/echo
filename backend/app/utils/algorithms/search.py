...
class TFIDFSearchEngine:
    ...
    def search(self, query: str, limit: int = 20, min_score: float = 0.1) -> List[Tuple[str, float]]:
        """
    ...
    def _calculate_phrase_bonus(self, doc_id: str, original_query: str, query_terms: List[str]) -> float:
        ...
        positions = {}
        for term in query_terms:
            positions[term] = [m.start() for m in re.finditer(r'\b' + re.escape(term) + r'\b', content)]
...
class SemanticSearchEngine:
    ...
    async def search(self, query_embedding: List[float], limit: int = 20, threshold: float = 0.7) -> List[Tuple[str, float]]:
...
class HybridSearchEngine:
    ...
    async def search(self, query: str, query_embedding: List[float], limit: int = 20) -> List[Tuple[str, float]]:
        """
