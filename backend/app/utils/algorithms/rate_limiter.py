        ...
        async with self.lock:
            ...
            if key not in self.buckets:
                ...
                return RateLimitResult(
                    allowed=True, remaining=self.capacity - 1, reset_time=now + self.window
                )
            ...
            if tokens >= 1:
                ...
                return RateLimitResult(
                    allowed=True, remaining=int(tokens), reset_time=now + self.window
                )
            else:
                self.buckets[key] = (tokens, now)
    ...
    async def is_allowed(self, key: str) -> RateLimitResult:
        ...
        async with self.lock:
            ...
            if key not in self.counters:
                ...
                return RateLimitResult(
                    allowed=True, remaining=self.limit - 1, reset_time=window_start + self.window
                )
...
class HierarchicalRateLimiter:
    ...
    async def is_allowed(self, keys: Dict[str, str]) -> RateLimitResult:
        ...
        if results:
            ...
            return RateLimitResult(allowed=True, remaining=min_remaining, reset_time=max_reset_time)
