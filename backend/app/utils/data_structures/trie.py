...
class Trie:
    ...
    def _collect_words(self, node: TrieNode, prefix: str, words: List[Tuple[str, int, Dict]]) -> None:
...
class AutocompleteService:
    ...
    def add_word(self, category: str, word: str, frequency: int = 1, metadata: Optional[Dict] = None) -> None:
    ...
    def get_suggestions(self, category: str, prefix: str, limit: int = 10) -> List[Dict]:
        ...
        logger.debug("Autocomplete suggestions generated", 
                    category=category, prefix=prefix, count=len(formatted_suggestions))
    ...
    def get_analytics(self) -> Dict:
        ...
        return {
            ...
            "category_stats": {
                category: trie.get_stats() 
                for category, trie in self.tries.items()
            },
            "cache_size": len(self.search_cache),
            "popular_searches": sorted(
                self.search_stats.items(), 
                key=lambda x: x[1], 
                reverse=True
            )[:10]
        }