...
from ..core.exceptions import APIException, NotFoundException, ValidationException
...
class ShopService:
    ...
    async def create_product(self, product_data: ProductCreate, seller_id: int) -> Product:
        ...
        product = Product(seller_id=seller_id, sku=sku, slug=slug, **product_data.model_dump())
    ...
    async def search_products(
        ...
        """Search products"""
        return await self.list_products(skip=skip, limit=limit, search=query, filters=filters)

    async def get_featured_products(self, limit: int = 10) -> List[Product]:
        """Get featured products"""
        result = await self.db.execute(
            select(Product)
            .where(and_(Product.status == ProductStatus.ACTIVE, Product.is_featured == True))
    ...
    async def create_order_from_cart(
        ...
        order_number = (
            f"ORD-{datetime.now(timezone.utc).strftime('%Y%m%d')}-{uuid.uuid4().hex[:8].upper()}"
        )
    ...
    async def update_order_status(
        self, order_id: int, status: OrderStatus, tracking_number: Optional[str] = None
    ) -> Order:
        """Update order status"""
    ...
    async def create_product_review(
        ...
        result = await self.db.execute(
            select(ProductReview).where(
                and_(
                    ProductReview.product_id == product_id, ProductReview.customer_id == customer_id
                )
            )
        )
    ...
    async def get_product_review_stats(self, product_id: int) -> Dict[str, Any]:
        ...
        result = await self.db.execute(
            ...
            ).where(and_(ProductReview.product_id == product_id, ProductReview.is_approved == True))
        )
        stats = result.first()
    ...
    async def get_sales_analytics(
        ...
        return {
            ...
            "completion_rate": (
                (stats.completed_orders / stats.total_orders * 100) if stats.total_orders else 0
            ),
        }
