...
class TuitionService:
    ...
    async def create_session(
        self, session_data: TuitionSessionCreate, teacher_id: int
    ) -> TuitionSession:
        ...
        session = TuitionSession(teacher_id=teacher_id, **session_data.model_dump())
    ...
    async def get_session_by_id(self, session_id: int) -> Optional[TuitionSession]:
        ...
        result = await self.db.execute(
            ...
            .options(
                selectinload(TuitionSession.attendances), selectinload(TuitionSession.assignments)
            )
    ...
    async def update_session(
        self, session_id: int, session_data: TuitionSessionUpdate
    ) -> TuitionSession:
        """Update session"""
    ...
    async def end_session(
        self, session_id: int, teacher_id: int, summary: Optional[str] = None
    ) -> TuitionSession:
        """End a session"""
        session = await self.get_session_by_id(session_id)
        if not session:
    ...
    async def get_student_sessions(
        ...
        enrolled_courses_query = select(Enrollment.course_id).where(
            and_(Enrollment.student_id == student_id, Enrollment.status == EnrollmentStatus.ACTIVE)
        )
    ...
    async def get_teacher_upcoming_sessions(
        self, teacher_id: int, limit: int = 10
    ) -> List[TuitionSession]:
    ...
    async def mark_bulk_attendance(
        self, session_id: int, attendance_data: List[AttendanceCreate]
    ...
    async def create_assignment(
        self, assignment_data: AssignmentCreate, teacher_id: int
    ) -> Assignment:
        ...
        assignment = Assignment(teacher_id=teacher_id, **assignment_data.model_dump())
    ...
    async def get_student_assignments(
        ...
        query = select(Assignment).where(
            and_(Assignment.course_id.in_(enrolled_course_ids), Assignment.is_published == True)
        )
    ...
    async def submit_assignment(
        ...
        existing_submission = await self.get_student_assignment_submission(
    ...
    async def create_study_material(
        self, material_data: StudyMaterialCreate, teacher_id: int
    ) -> StudyMaterial:
        ...
        material = StudyMaterial(teacher_id=teacher_id, **material_data.model_dump())
    ...
    async def get_study_material_by_id(self, material_id: int) -> Optional[StudyMaterial]:
        ...
        result = await self.db.execute(select(StudyMaterial).where(StudyMaterial.id == material_id))
        return result.scalar_one_or_none()
    ...
    async def list_study_materials(
        ...
        if user_role == "student":
            ...
            enrolled_courses_query = select(Enrollment.course_id).where(
                and_(Enrollment.student_id == user_id, Enrollment.status == EnrollmentStatus.ACTIVE)
            )
            enrolled_courses_result = await self.db.execute(enrolled_courses_query)
            enrolled_course_ids = [row[0] for row in enrolled_courses_result.fetchall()]
            ...
            query = query.where(
                or_(StudyMaterial.teacher_id == user_id, StudyMaterial.is_public == True)
            )
    ...
    async def create_quiz(self, quiz_data: QuizCreate, teacher_id: int) -> Quiz:
        ...
        quiz = Quiz(teacher_id=teacher_id, **quiz_data.model_dump(exclude={"questions"}))
    ...
    async def submit_quiz_attempt(self, attempt_id: int, answers: Dict[str, Any]) -> QuizAttempt:
        ...
        attempt.percentage = (
            (correct_answers / quiz.total_points * 100) if quiz.total_points > 0 else 0
        )
    ...
    async def generate_progress_report(
        ...
        attendance_stats = await self._calculate_attendance_stats(
            enrollment.student_id, enrollment.course_id, start_date, end_date
        )
    ...
    async def get_course_analytics(
        self, course_id: int, start_date: Optional[date] = None, end_date: Optional[date] = None
    ) -> Dict[str, Any]:
        """Get course analytics"""
        # Session analytics
        session_query = select(
            func.count(TuitionSession.id).label("total_sessions"),
            func.count(TuitionSession.id)
            .filter(TuitionSession.status == SessionStatus.COMPLETED)
        ...
        attendance_query = (
            select(
                ...
                .filter(SessionAttendance.status == AttendanceStatus.PRESENT)
                .label("present_count"),
        ...
        return {
            "sessions": {
                ...
                "completion_rate": (
                    (session_stats.completed_sessions / session_stats.total_sessions * 100)
            ...
            "attendance": {
                ...
                "attendance_rate": (
                    (
                        ...
                        / attendance_stats.total_attendance_records
                        * 100
                    )
                    if attendance_stats.total_attendance_records
                    else 0
                ),
    ...
    async def get_student_analytics(
        ...
        if course_id:
            attendance_query = attendance_query.join(TuitionSession).where(
        ...
        assignment_query = select(
            ...
            .filter(AssignmentSubmission.status == AssignmentStatus.GRADED)
        ...
        if course_id:
            assignment_query = assignment_query.join(Assignment).where(
        ...
        return {
            ...
            "attendance": {
                ...
                "attendance_rate": (
                    (attendance_stats.attended_sessions / attendance_stats.total_sessions * 100)
                    if attendance_stats.total_sessions
                    else 0
                ),
                "average_participation": float(attendance_stats.avg_participation or 0),
            },
    ...
    async def check_assignment_access(
        self, assignment_id: int, user_id: int, user_role: str
    ) -> bool:
    ...
    async def check_material_access(self, material_id: int, user_id: int, user_role: str) -> bool:
        ...
        if user_role == "admin":
            ...
            if material.requires_enrollment:
                enrollment = await self._get_student_enrollment_by_course(
                    user_id, material.course_id
                )
                return enrollment is not None and enrollment.status == EnrollmentStatus.ACTIVE
        ...
        query = select(TuitionSession).where(
            and_(
                ...
                or_(
                    ...
                    and_(TuitionSession.start_time < end_time, TuitionSession.end_time >= end_time),
                    and_(
                        TuitionSession.start_time >= start_time, TuitionSession.end_time <= end_time
                    ),
                ),
            )
        )
    ...
    async def _get_student_enrollment_by_course(
    ...
    async def _calculate_attendance_stats(
        self, student_id: int, course_id: int, start_date: datetime, end_date: datetime
    ) -> Dict[str, Any]:
