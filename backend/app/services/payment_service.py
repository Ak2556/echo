...
class PaymentService:
    ...
    async def create_payment_intent(
        ...
        transaction_id = (
            f"TXN-{datetime.now(timezone.utc).strftime('%Y%m%d')}-{uuid.uuid4().hex[:8].upper()}"
        )
    ...
    async def confirm_payment(
        self, payment_id: int, gateway_payment_id: str, gateway_order_id: Optional[str] = None
    ) -> Payment:
        """Confirm a payment"""
    ...
    async def fail_payment(self, payment_id: int, reason: Optional[str] = None) -> Payment:
    ...
    async def refund_payment(self, payment_id: int, refund_amount: float, reason: str) -> Payment:
        """Process a refund"""
        payment = await self.db.get(Payment, payment_id)
        if not payment:
            raise NotFoundException("Payment not found")
