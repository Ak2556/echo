...
class User:
    ...
    def is_active(self) -> bool:
        ...
        return (
            ...
            (self.locked_until is None or self.locked_until < datetime.now(timezone.utc))
        )
    ...
    def can_perform_action(self, action: str) -> bool:
        ...
        permissions = {
            UserRole.USER: {
                "create_post", "like_post", "comment", "follow_user",
                "update_profile", "delete_own_content"
            },
            UserRole.MODERATOR: {
                "create_post", "like_post", "comment", "follow_user",
                "update_profile", "delete_own_content", "moderate_content",
                "ban_user", "delete_any_content"
            },
            UserRole.ADMIN: {
                "create_post", "like_post", "comment", "follow_user",
                "update_profile", "delete_own_content", "moderate_content",
                "ban_user", "delete_any_content", "manage_users",
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'User':
        ...
        datetime_fields = ["created_at", "updated_at", "last_login", "last_password_change", "locked_until"]
        ...
        if "stats" in data:
            ...
            if "last_active" in stats_data and stats_data["last_active"]:
                if isinstance(stats_data["last_active"], str):
                    stats_data["last_active"] = datetime.fromisoformat(stats_data["last_active"].replace('Z', '+00:00'))
            data["stats"] = UserStats(**{k: v for k, v in stats_data.items() if hasattr(UserStats, k)})
