...
async def search_products(
    ...
    products = await shop_service.search_products(query=q, filters=filters, skip=skip, limit=limit)
    return products
...
async def get_related_products(
    product_id: int, limit: int = Query(5, ge=1, le=20), db: AsyncSession = Depends(get_db)
):
...
async def create_product(
    ...
    if current_user.role not in ["teacher", "admin"]:  # Teachers can sell products
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Only sellers can create products"
        )

    product = await shop_service.create_product(
        product_data=product_data, seller_id=current_user.id
    )
    return product
...
async def update_product(
    ...
    if product.seller_id != current_user.id and current_user.role != "admin":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to update this product"
        )
...
async def delete_product(
    ...
    if product.seller_id != current_user.id and current_user.role != "admin":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to delete this product"
        )
...
async def add_to_cart(
    ...
    cart_item = await shop_service.add_to_cart(
        user_id=current_user.id, product_id=item_data.product_id, quantity=item_data.quantity
    )
...
async def update_cart_item(
    ...
    cart_item = await shop_service.update_cart_item(
        user_id=current_user.id, product_id=product_id, quantity=item_data.quantity
    )
...
async def remove_from_cart(
    ...
    await shop_service.remove_from_cart(user_id=current_user.id, product_id=product_id)
...
async def create_order(
    ...
    payment_intent = await payment_service.create_payment_intent(
        amount=order.total_amount, currency="INR", order_id=order.id, customer_id=current_user.id
    )
...
async def get_order(
    ...
    if order.customer_id != current_user.id and current_user.role != "admin":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to view this order"
        )
...
async def cancel_order(
    ...
    if order.customer_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to cancel this order"
        )
...
async def get_product_reviews(
    ...
    reviews = await shop_service.get_product_reviews(
        product_id=product_id, skip=skip, limit=limit, rating_filter=rating
    )
...
async def get_product_review_stats(product_id: int, db: AsyncSession = Depends(get_db)):
...
async def admin_list_products(
    ...
    products = await shop_service.list_products(
        skip=skip, limit=limit, filters=filters, include_inactive=True
    )
...
async def admin_list_orders(
    ...
    orders = await shop_service.list_orders(
        skip=skip, limit=limit, status=order_status, customer_id=customer_id
    )
...
async def admin_update_order_status(
    ...
    order = await shop_service.update_order_status(
        order_id=order_id, status=order_status, tracking_number=tracking_number
    )
...
async def get_sales_analytics(
    ...
    analytics = await shop_service.get_sales_analytics(start_date=start_date, end_date=end_date)
