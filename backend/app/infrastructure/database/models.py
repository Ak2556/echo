...
from ...domain.entities.post import PostStatus, PostType, PostVisibility
...
class UserModel(Base):
    ...
    # Account management
    status = Column(
        SQLEnum(UserStatus), nullable=False, default=UserStatus.PENDING_VERIFICATION, index=True
    )
    ...
    liked_posts = relationship(
        "PostModel", secondary=post_likes, back_populates="liked_by_users", lazy="dynamic"
    )

    disliked_posts = relationship(
        "PostModel", secondary=post_dislikes, back_populates="disliked_by_users", lazy="dynamic"
    )

    saved_posts = relationship(
        "PostModel", secondary=post_saves, back_populates="saved_by_users", lazy="dynamic"
    )
...
class PostModel(Base):
    ...
    visibility = Column(
        SQLEnum(PostVisibility), nullable=False, default=PostVisibility.PUBLIC, index=True
    )
    ...
    liked_by_users = relationship(
        "UserModel", secondary=post_likes, back_populates="liked_posts", lazy="dynamic"
    )

    disliked_by_users = relationship(
        "UserModel", secondary=post_dislikes, back_populates="disliked_posts", lazy="dynamic"
    )

    saved_by_users = relationship(
        "UserModel", secondary=post_saves, back_populates="saved_posts", lazy="dynamic"
    )
