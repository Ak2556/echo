...
class TestProductEndpoints:
    ...
    async def test_list_products_success(
        self, client: AsyncClient, mock_shop_service, sample_product
    ):
    ...
    async def test_search_products(self, client: AsyncClient, mock_shop_service, sample_product):
        ...
        with patch("app.api.v1.endpoints.shop.ShopService", return_value=mock_shop_service):
            response = await client.get("/api/v1/shop/products/search", params={"q": "test"})
    ...
    async def test_search_products_with_category(
        ...
        with patch("app.api.v1.endpoints.shop.ShopService", return_value=mock_shop_service):
            response = await client.get(
                ...
                params={"q": "test", "category": ProductCategory.BOOKS.value},
            )
    ...
    async def test_create_product_as_seller(
        self, client: AsyncClient, mock_shop_service, seller_user, sample_product, override_auth
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_product_as_student_forbidden(
        self, client: AsyncClient, mock_shop_service, student_user, override_auth
    ):
    ...
    async def test_update_product_not_owner(
        self, client: AsyncClient, mock_shop_service, student_user, sample_product, override_auth
    ):
    ...
    async def test_update_product_not_found(
        self, client: AsyncClient, mock_shop_service, seller_user, override_auth
    ):
    ...
    async def test_delete_product_not_found(
        ...
        with patch("app.api.v1.endpoints.shop.ShopService", return_value=mock_shop_service):
            with override_auth(seller_user):
                response = await client.delete("/api/v1/shop/products/999")
...
class TestOrderEndpoints:
    ...
    async def test_create_order(
        ...
        with patch("app.api.v1.endpoints.shop.ShopService", return_value=mock_shop_service):
            with patch(
                "app.api.v1.endpoints.shop.PaymentService", return_value=mock_payment_service
            ):
                with override_auth(student_user):
    ...
    async def test_get_user_orders_with_status_filter(
        ...
        with patch("app.api.v1.endpoints.shop.ShopService", return_value=mock_shop_service):
            with override_auth(student_user):
                response = await client.get(
                    "/api/v1/shop/orders", params={"status": OrderStatus.DELIVERED.value}
                )

        assert response.status_code == status.HTTP_200_OK

    @pytest.mark.asyncio
    async def test_get_order_as_owner(
        self, client: AsyncClient, mock_shop_service, student_user, override_auth
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_order_not_owner(
        self, client: AsyncClient, mock_shop_service, seller_user, override_auth
    ):
    ...
    @pytest.mark.asyncio
    async def test_cancel_order_as_owner(
        self, client: AsyncClient, mock_shop_service, student_user, override_auth
    ):
    ...
    async def test_cancel_order_not_found(
        ...
        with patch("app.api.v1.endpoints.shop.ShopService", return_value=mock_shop_service):
            with override_auth(student_user):
                response = await client.post("/api/v1/shop/orders/999/cancel")
...
class TestReviewEndpoints:
    ...
    async def test_get_product_reviews_with_rating_filter(
        self, client: AsyncClient, mock_shop_service
    ):
        """Test getting product reviews with rating filter"""
        mock_shop_service.get_product_reviews = AsyncMock(return_value=[])

        with patch("app.api.v1.endpoints.shop.ShopService", return_value=mock_shop_service):
            response = await client.get("/api/v1/shop/products/1/reviews", params={"rating": 5})
    ...
    async def test_create_product_review_success(
        ...
        review_data = {"rating": 5, "title": "Great!", "comment": "Excellent product"}
    ...
    @pytest.mark.asyncio
    async def test_create_product_review_not_purchased(
        self, client: AsyncClient, mock_shop_service, student_user, override_auth
    ):
...
class TestAdminEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_admin_list_products_success(
        self, client: AsyncClient, mock_shop_service, admin_user, sample_product, override_auth
    ):
    ...
    @pytest.mark.asyncio
    async def test_admin_list_products_non_admin(
        self, client: AsyncClient, mock_shop_service, student_user, override_auth
    ):
    ...
    @pytest.mark.asyncio
    async def test_admin_list_products_with_filters(
        self, client: AsyncClient, mock_shop_service, admin_user, sample_product, override_auth
    ):
        ...
        with patch("app.api.v1.endpoints.shop.ShopService", return_value=mock_shop_service):
            with override_auth(admin_user):
                response = await client.get(
                    ...
                    params={"seller_id": 1, "product_status": ProductStatus.ACTIVE.value},
                )

        assert response.status_code == status.HTTP_200_OK
        # Verify the filters were passed correctly
        call_args = mock_shop_service.list_products.call_args
        assert call_args.kwargs["filters"]["seller_id"] == 1
        assert call_args.kwargs["filters"]["status"] == ProductStatus.ACTIVE

    @pytest.mark.asyncio
    async def test_admin_list_orders_success(
        self, client: AsyncClient, mock_shop_service, admin_user, override_auth
    ):
    ...
    @pytest.mark.asyncio
    async def test_admin_list_orders_non_admin(
        self, client: AsyncClient, mock_shop_service, student_user, override_auth
    ):
    ...
    @pytest.mark.asyncio
    async def test_admin_update_order_status_success(
        self, client: AsyncClient, mock_shop_service, admin_user, override_auth
    ):
        ...
        with patch("app.api.v1.endpoints.shop.ShopService", return_value=mock_shop_service):
            with override_auth(admin_user):
                response = await client.put(
                    ...
                    params={"status": OrderStatus.SHIPPED.value, "tracking_number": "TRACK123"},
                )
    ...
    @pytest.mark.asyncio
    async def test_admin_update_order_status_non_admin(
        self, client: AsyncClient, mock_shop_service, student_user, override_auth
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_sales_analytics_success(
        self, client: AsyncClient, mock_shop_service, admin_user, override_auth
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_sales_analytics_with_date_range(
        self, client: AsyncClient, mock_shop_service, admin_user, override_auth
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_sales_analytics_non_admin(
        self, client: AsyncClient, mock_shop_service, student_user, override_auth
    ):
