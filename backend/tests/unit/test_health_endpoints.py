...
class TestHealthEndpoints:
    ...
    async def test_readiness_check_healthy(self):
        ...
        with (
            patch("app.api.v1.endpoints.health.db_health_check") as mock_db_health,
    ...
    @pytest.mark.asyncio
    async def test_readiness_check_ai_service_exception(self):
        """Test readiness check handles AI service exception."""
        from app.api.v1.endpoints.health import readiness_check
        ...
        with (
            ...
            mock_db_health.return_value = True
            # First access returns "test_key", second access raises exception
            access_count = [0]

            def api_key_getter():
                access_count[0] += 1
                if access_count[0] == 1:
                    return "test_key"  # For line 78 check
                raise Exception("API key error")  # For line 83 access

            type(mock_settings).openrouter_api_key = property(lambda self: api_key_getter())
            mock_settings.feature_ai_chat = True
            mock_settings.app_version = "1.0.0"
...
class TestHealthEndpointIntegration:
    ...
    async def test_health_detailed_ai_service_exception(self):
        ...
        with patch("app.api.v1.endpoints.health.settings") as mock_settings:
            ...
            type(mock_settings).openrouter_api_key = property(
                lambda self: (_ for _ in ()).throw(Exception("API key error"))
            )
            mock_settings.app_version = "1.0.0"
            mock_settings.environment = "test"
